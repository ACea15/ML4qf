...
def mean_variance_opt(mu, Sigma_inv, lambda_portfolio):
    w_opt = 1.0 / lambda_portfolio * Sigma_inv @ mu
    return w_opt
...
class MinimizeFuns:
    ...
    def mean_variance(x, *args):
        ...
        return -portfolio_mean_return + 0.5 * lambda_portfolio * portfolio_variance
    ...
    def sharpe(x, *args):
        ...
        portfolio_mean_return = np.dot(mu, x)
        portfolio_variance = np.dot(x, Sigma @ x)
        sharpe_ratio = (-portfolio_mean_return + risk_free) / np.sqrt(portfolio_variance)
        return sharpe_ratio
    ...
    **kwargs,
    ...
    if cons_sett is not None:
        ...
        for k, v in cons_sett.items():
            ...
            c_dict = dict(fun=c_fun, type=c_type, jac=c_jac, args=args)
            constraints.append(c_dict)
    else:
        constraints = ()

    res = scipy.optimize.minimize(
        fun, x0, args, method=method_name, constraints=constraints, **kwargs
    )
    return res
